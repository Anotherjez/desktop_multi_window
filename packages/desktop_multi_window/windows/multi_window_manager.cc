...
{
  flutter::EncodableMap empty_config;
  return Create(std::move(args), empty_config);
}

int64_t MultiWindowManager::Create(std::string args, const flutter::EncodableMap &transparency_config)
{
  g_next_id_++;
  int64_t id = g_next_id_;

  auto window = std::make_unique<FlutterWindow>(id, std::move(args), shared_from_this(), transparency_config);
...
void MultiWindowManager::SetTransparency(int64_t id, const flutter::EncodableMap &transparency_config)
{
  auto window = windows_.find(id);
  if (window != windows_.end())
  {
    window->second->SetTransparency(transparency_config);
  }
}
