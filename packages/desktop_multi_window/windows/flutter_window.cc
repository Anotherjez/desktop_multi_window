...
FlutterWindow::FlutterWindow(
    int64_t id,
    std::string args,
    const std::shared_ptr<FlutterWindowCallback> &callback) : FlutterWindow(id, std::move(args), callback, flutter::EncodableMap())
{
}
...
FlutterWindow::FlutterWindow(
    ...
    const flutter::EncodableMap &transparency_config) : callback_(callback), id_(id), window_handle_(nullptr), scale_factor_(1)
{
  ...
  // Determine extended window styles based on transparency config
  DWORD exStyle = WS_EX_APPWINDOW;

  // Extract transparency parameters for initial window creation
  int mode = 0;
...
// Here we attempt a weak link call when available.
  ...
  // Apply transparency settings if configured
  if (mode > 0)
  {
    SetTransparency(transparency_config);
  }
